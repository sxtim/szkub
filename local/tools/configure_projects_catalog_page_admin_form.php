<?php
/**
 * Настраивает форму редактирования singleton-ИБ projects_catalog_page.
 *
 * CLI:
 *   php local/tools/configure_projects_catalog_page_admin_form.php --dry-run=1
 *   php local/tools/configure_projects_catalog_page_admin_form.php --dry-run=0
 */
...
if (PHP_SAPI === "cli") {
	...
	if (isset($options["help"])) {
		echo "Usage: php local/tools/configure_projects_catalog_page_admin_form.php [--admin-user-id=1] [--dry-run=1]\n";
		exit(0);
	}
...
function projectsCatalogPageAdminFindIblock($code)
{
...
function projectsCatalogPageAdminPropertyMap($iblockId)
{
	$result = array();
...
$iblock = projectsCatalogPageAdminFindIblock("projects_catalog_page");
if (!is_array($iblock)) {
	echo "[ERROR] IBlock not found by code: projects_catalog_page" . PHP_EOL;
	exit(2);
}

$iblockId = (int)$iblock["ID"];
$formId = "form_element_" . $iblockId;
$propertyMap = projectsCatalogPageAdminPropertyMap($iblockId);
...
if (isset($propertyMap["INTRO_TEXT"])) {
	$fields["PROPERTY_" . (int)$propertyMap["INTRO_TEXT"]["ID"]] = "Вступительный текст";
}
...
echo "[OK] Saved layout for projects_catalog_page" . PHP_EOL;
