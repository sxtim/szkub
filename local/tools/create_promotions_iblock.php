<?php
/**
 * Идемпотентное создание инфоблока "Акции" и его свойств.
 *
 * CLI:
 *   php local/tools/create_promotions_iblock.php
 */
...
if (PHP_SAPI === "cli") {
	...
	if (isset($options["help"])) {
		echo "Usage: php local/tools/create_promotions_iblock.php [--site-id=s1] [--type-id=content] [--iblock-code=promotions]\n";
		exit(0);
	}
...
$iblockCode = isset($_REQUEST["iblock_code"]) && $_REQUEST["iblock_code"] !== "" ? (string)$_REQUEST["iblock_code"] : "promotions";
$iblockName = isset($_REQUEST["iblock_name"]) && $_REQUEST["iblock_name"] !== "" ? (string)$_REQUEST["iblock_name"] : "Акции";
...
if ($row = $iblockRes->Fetch()) {
	...
	$iblockFields = array(
		...
		"SECTION_PAGE_URL" => "#SITE_DIR#/promotions/",
