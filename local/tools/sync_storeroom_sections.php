...
if (PHP_SAPI === "cli") {
    ...
    if (isset($options["help"])) {
        echo "Usage: php local/tools/sync_storeroom_sections.php [--storerooms-code=storerooms] [--projects-code=projects] [--dry-run=1]\n";
        exit(0);
    }
...
function storeroomSectionsFindIblockByCode($code)
{
    $res = CIBlock::GetList(array(), array("=CODE" => (string)$code), false);
    return $res ? $res->Fetch() : false;
}

function storeroomSectionsTopSectionsByCode($iblockId)
{
    $result = array();
...
function storeroomSectionsElementSectionIds($elementId)
{
    $ids = array();
...
function storeroomSectionsEnsureSectionsMode($iblockId, $dryRun)
{
    ...
    $needsSync =
        ...
        || (string)$row["SECTION_PAGE_URL"] !== "#SITE_DIR#/storerooms/#SECTION_CODE_PATH#/";
    ...
    return (bool)$ib->Update($iblockId, array(
        ...
        "LIST_PAGE_URL" => "#SITE_DIR#/storerooms/",
        "SECTION_PAGE_URL" => "#SITE_DIR#/storerooms/#SECTION_CODE_PATH#/",
        "DETAIL_PAGE_URL" => "#SITE_DIR#/storerooms/#ELEMENT_CODE#/",
        "SECTION_CHOOSER" => "L",
        "SECTIONS" => "Y",
...
$storeroomsCode = isset($_REQUEST["storerooms_code"]) && $_REQUEST["storerooms_code"] !== "" ? (string)$_REQUEST["storerooms_code"] : "storerooms";
...
echo "Storerooms iblock code: " . $storeroomsCode . PHP_EOL;
echo "Projects iblock code: " . $projectsCode . PHP_EOL;
echo "dry-run: " . ($dryRun ? "Y" : "N") . PHP_EOL;

$storeroomsIblock = storeroomSectionsFindIblockByCode($storeroomsCode);
if (!is_array($storeroomsIblock) || (int)$storeroomsIblock["ID"] <= 0) {
    echo "[ERROR] Storerooms iblock not found by code: " . $storeroomsCode . PHP_EOL;
    exit(2);
}

$projectsIblock = storeroomSectionsFindIblockByCode($projectsCode);
...
if (!storeroomSectionsEnsureSectionsMode($storeroomsIblockId, $dryRun)) {
    echo "[ERROR] Failed to enable sections mode for storerooms iblock" . PHP_EOL;
    exit(3);
}

$existingSections = storeroomSectionsTopSectionsByCode($storeroomsIblockId);
...
$storeroomsRes = CIBlockElement::GetList(
    array("SORT" => "ASC", "ID" => "ASC"),
    array("IBLOCK_ID" => $storeroomsIblockId, "ACTIVE" => "Y"),
    false,
    false,
    array("ID", "NAME", "IBLOCK_SECTION_ID", "IBLOCK_ID")
);

while ($storeroom = $storeroomsRes->GetNextElement()) {
    $fields = $storeroom->GetFields();
    $properties = $storeroom->GetProperties();
    ...
    $currentSectionIds = storeroomSectionsElementSectionIds($elementId);
...
echo "[OK] Storeroom sections sync finished" . PHP_EOL;
