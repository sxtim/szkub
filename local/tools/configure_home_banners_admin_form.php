<?php
/**
 * Настраивает форму редактирования элемента ИБ home_banners в админке Bitrix.
 *
 * CLI:
 *   php local/tools/configure_home_banners_admin_form.php --dry-run=1
 *   php local/tools/configure_home_banners_admin_form.php --dry-run=0
 */
...
if (PHP_SAPI === "cli") {
	...
	if (isset($options["help"])) {
		echo "Usage: php local/tools/configure_home_banners_admin_form.php [--code=home_banners] [--admin-user-id=1] [--dry-run=1]" . PHP_EOL;
		exit(0);
	}
...
$iblockCode = isset($_REQUEST["code"]) && $_REQUEST["code"] !== "" ? (string)$_REQUEST["code"] : "home_banners";
...
if (isset($propertyMap["HIDE_TITLE"])) {
	$fields["PROPERTY_" . (int)$propertyMap["HIDE_TITLE"]["ID"]] = "Не выводить название";
}
...
if (isset($propertyMap["SLOT"])) {
	$fields["PROPERTY_" . (int)$propertyMap["SLOT"]["ID"]] = (string)$propertyMap["SLOT"]["NAME"];
}
if (isset($propertyMap["LINK_URL"])) {
	$fields["PROPERTY_" . (int)$propertyMap["LINK_URL"]["ID"]] = (string)$propertyMap["LINK_URL"]["NAME"];
}
if (isset($propertyMap["LINK_TARGET"])) {
	$fields["PROPERTY_" . (int)$propertyMap["LINK_TARGET"]["ID"]] = (string)$propertyMap["LINK_TARGET"]["NAME"];
}

$fields += array(
	"PREVIEW_TEXT" => "Текст баннера",
