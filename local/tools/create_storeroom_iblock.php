...
if (PHP_SAPI === "cli") {
    ...
    if (isset($options["help"])) {
        echo "Usage: php local/tools/create_storeroom_iblock.php [--site-id=s1] [--type-id=realty] [--iblock-code=storerooms]\n";
        exit(0);
    }
...
function storeroomEnsureEnumValues($propertyId, array $values)
{
...
$iblockCode = isset($_REQUEST["iblock_code"]) && $_REQUEST["iblock_code"] !== "" ? (string)$_REQUEST["iblock_code"] : "storerooms";
$iblockName = isset($_REQUEST["iblock_name"]) && $_REQUEST["iblock_name"] !== "" ? (string)$_REQUEST["iblock_name"] : "Кладовые";
...
foreach ($properties as $propertyDef) {
    ...
    if (isset($propertyDef["VALUES"]) && is_array($propertyDef["VALUES"])) {
        storeroomEnsureEnumValues($propertyId, $propertyDef["VALUES"]);
    }
}
