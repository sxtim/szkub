...
if (PHP_SAPI === "cli") {
    ...
    if (isset($options["help"])) {
        echo "Usage: php local/tools/sync_commercial_sections.php [--commercial-code=commercial] [--projects-code=projects] [--dry-run=1]\n";
        exit(0);
    }
...
function commercialSectionsFindIblockByCode($code)
{
    $res = CIBlock::GetList(array(), array("=CODE" => (string)$code), false);
    return $res ? $res->Fetch() : false;
}

function commercialSectionsTopSectionsByCode($iblockId)
{
    $result = array();
...
function commercialSectionsElementSectionIds($elementId)
{
    $ids = array();
...
function commercialSectionsEnsureSectionsMode($iblockId, $dryRun)
{
...
$commercialCode = isset($_REQUEST["commercial_code"]) && $_REQUEST["commercial_code"] !== "" ? (string)$_REQUEST["commercial_code"] : "commercial";
...
echo "Commercial iblock code: " . $commercialCode . PHP_EOL;
echo "Projects iblock code: " . $projectsCode . PHP_EOL;
echo "dry-run: " . ($dryRun ? "Y" : "N") . PHP_EOL;

$commercialIblock = commercialSectionsFindIblockByCode($commercialCode);
if (!is_array($commercialIblock) || (int)$commercialIblock["ID"] <= 0) {
    echo "[ERROR] Commercial iblock not found by code: " . $commercialCode . PHP_EOL;
    exit(2);
}

$projectsIblock = commercialSectionsFindIblockByCode($projectsCode);
...
if (!commercialSectionsEnsureSectionsMode($commercialIblockId, $dryRun)) {
    echo "[ERROR] Failed to enable sections mode for commercial iblock" . PHP_EOL;
    exit(3);
}

$existingSections = commercialSectionsTopSectionsByCode($commercialIblockId);
...
$commercialRes = CIBlockElement::GetList(
    array("SORT" => "ASC", "ID" => "ASC"),
    array("IBLOCK_ID" => $commercialIblockId, "ACTIVE" => "Y"),
    false,
    false,
    array("ID", "NAME", "IBLOCK_SECTION_ID", "IBLOCK_ID")
);

while ($commercial = $commercialRes->GetNextElement()) {
    $fields = $commercial->GetFields();
    $properties = $commercial->GetProperties();
    ...
    $currentSectionIds = commercialSectionsElementSectionIds($elementId);
...
echo "[OK] Commercial sections sync finished" . PHP_EOL;
